...
def fetch_daily_consumption(
    ...
    session.headers["User-Agent"] = "octopus-to-tado-backfill/3.0"
...
def fill_isolated_daily_gaps(
    records: list[DailyConsumption],
    max_gap_days: int = 1,
    neighbour_window_days: int = 3,
) -> tuple[list[DailyConsumption], list[str]]:
    """
    Estimate isolated whole-day gaps from nearby reported daily consumption.

    Only gaps of at most max_gap_days are filled. Each missing day receives
    the mean of non-imputed daily records whose local start date is within
    neighbour_window_days. Longer or irregular gaps remain unfilled and will
    continue to block upload.
    """
    ordered = sorted(records, key=lambda row: (row.start, row.end))
    actual = [row for row in ordered if not row.imputed]
    additions: list[DailyConsumption] = []
    messages: list[str] = []
    ...
    for previous, current in zip(ordered, ordered[1:]):
        ...
        cursor = previous.end.astimezone(LOCAL_TZ)
        gap_end = current.start.astimezone(LOCAL_TZ)
        ...
        while cursor < gap_end:
            ...
            if next_day > gap_end:
                missing_intervals = []
                break
            missing_intervals.append((cursor, next_day))
            cursor = next_day

        if not missing_intervals or len(missing_intervals) > max_gap_days:
            continue
        if cursor != gap_end:
            continue

        for missing_start, missing_end in missing_intervals:
            missing_day = missing_start.date()
            nearby = [
                row.consumption
                for row in actual
                if abs(
                    (row.start.astimezone(LOCAL_TZ).date() - missing_day).days
                ) <= neighbour_window_days
            ]
            if not nearby:
                continue

            estimate = (
                sum(nearby, Decimal("0")) / Decimal(len(nearby))
            ...
            messages.append(
                f"{missing_day.isoformat()}: estimated {estimate} m3 "
                f"from {len(nearby)} nearby reported day(s)"
            )

    combined = ordered + additions
    combined.sort(key=lambda row: (row.start, row.end))
...
def build_plan(
    ...
    bucket_imputed_days: list[str] = []

    for row in days:
        running += row.consumption
        bucket_consumption += row.consumption
        if row.imputed:
            bucket_imputed_days.append(
                row.start.astimezone(LOCAL_TZ).date().isoformat()
            )
        ...
        if is_monday_boundary and local_end.date() < anchor_day:
            ...
            note = "weekly"
            if bucket_imputed_days:
                note += "; includes estimated day(s): " + ", ".join(
                    bucket_imputed_days
                )
            if extra:
                note += f"; {extra}"
            ...
            candidates[local_end.date()] = PlanRow(
                ...
                status=status,
                note=note,
                imputed_days=tuple(bucket_imputed_days),
            )
    ...
    if bucket_imputed_days:
        anchor_note += "; includes estimated day(s): " + ", ".join(
            bucket_imputed_days
        )
    if anchor_extra:
            ...
            writer.writerow(
                {
                    ...
                    "status": row.status,
                    "note": row.note,
                    "imputed_days": ",".join(row.imputed_days),
...
def upload(
    ...
    for index, row in enumerate(rows, 1):
        # Use positional arguments for compatibility with python-tado 0.19.2
        # and newer PyTado versions. The parameter names changed between
        # releases, but the order remains (date, reading).
        # PyTado 0.19.2 expects the Energy IQ date as a YYYY-MM-DD
        # string. A datetime.date object is not JSON serialisable.
        result = method(row.day.isoformat(), row.integer)
        print(
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Estimate isolated one-day Octopus gaps from nearby daily "
            "consumption. Longer gaps remain blocked."
        ),
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if coverage.gaps and args.fill_isolated_gaps:
        days, imputed_messages = fill_isolated_daily_gaps(days)
        days, coverage = select_and_check_coverage(
            days, requested_start, anchor_boundary
        )
        print(f"Imputed isolated gaps: {len(imputed_messages)}")
        for message in imputed_messages:
            print(f"  {message}")
